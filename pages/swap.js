...
function Swap() {
  ...
  const {
      ...
      ),
    {
      ...
      onSuccess: (data) => {
        setTokenTwoAmount(
          (Number(data.price) / 10 ** tokenTwo.decimals).toFixed(2)
        );
      },
    }
  );
  ...
  async function fetchDexSwap() {
    ...
    const allowance = await axios.get(
      `https://api.1inch.io/v5.0/42161/approve/allowance?tokenAddress=${tokenOne.address}&walletAddress=${address}`
    );
    ...
    const tx = await axios.get(
      `https://api.1inch.io/v5.0/42161/swap?fromTokenAddress=${
        tokenOne.address
  ...
  return (
    <div className='overflow-hidden pt-[80px] flex flex-col space-y-10 items-center md:h-screen'>
      ...
      <div className='w-[80%]  border-2 border-purple max-w-[500px] space-y-10 rounded-[30px] p-8'>
        <div>
          <p className='text-white font-bold font-press mb-2'>
            Slippage Tolerance:
          </p>
          <Slippage
            slippage={slippage}
        ...
        <div className='flex justify-center mt-10'>
          {!isConnected ? (
            ...
            <button
              ...
              className='rounded-full bg-purple/30 flex items-center p-2 space-x-2 px-5 text-white font-bold'
            >
              Swap
            </button>
          )}
